...
extension AstParser on AstNode {
  ...
  CompilationUnit? get compilationUnit {
    final result = thisOrAncestorMatching((node) => node is CompilationUnit);

    if (result is CompilationUnit) return result;
...
extension CompilationUnitX on CompilationUnit {
  List<ImportDirective> importDirectives() {
    return directives.whereType<ImportDirective>().toList();
  }
}
